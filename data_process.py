...
def get_data_test(path_mix, path_t1, path_t2, num_speaker, batch_size, time_step):
    ...
    for speaker in filelist:    # for each speaker
        ...
        for sentence in sentlist:    # for each sentence in each speaker file
            ...
            tmp     = sio.loadmat(path_mix+speaker+'/'+sentence)
            tmp_out = tmp.get('mix_train')
            tmp_out, sequence = zeros_padding(tmp_out, batch_size, time_step)
            mix[speaker].update({sentence:tmp_out.T})
            # append target 1
            tmp     = sio.loadmat(path_t1+speaker+'/'+sentence)
            tmp_out = tmp.get('target1')
            tmp_out, _ = zeros_padding(tmp_out, batch_size, time_step)
            t1[speaker].update({sentence:tmp_out.T})
            # append target 2
            tmp     = sio.loadmat(path_t2+speaker+'/'+sentence)
            tmp_out = tmp.get('target2')
            tmp_out, _ = zeros_padding(tmp_out, batch_size, time_step)
            t2[speaker].update({sentence:tmp_out.T})
            # sequence_length
            sequence_length[speaker].update({sentence:sequence})
    return mix, t1, t2, sequence_length


def zeros_padding(x, batch_size, time_step):
    input_size, input_step = x.shape
    padding_size = time_step - input_step
    x_out = np.lib.pad(x, ((0, 0), (0, padding_size)), 'constant')
    ...
    return x_out, sequence_length
"""
def zeros_padding(x, batch_size, time_step):
    input_size, data_size = x.shape
